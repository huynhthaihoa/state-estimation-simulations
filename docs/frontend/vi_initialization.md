...
> **Note**: Structure from Motion (SfM) is a computer-vision technique that recovers camera poses and sparse 3D point positions from a sequence of 2D images taken from different viewpoints. For a single (monocular) camera, the recovered translations and structure are correct only up to an unknown scale factor - rotations are not - which is exactly why §1 lists scale, not rotation, as an unknown.
