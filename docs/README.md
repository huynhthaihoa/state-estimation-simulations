...
- [hybrid_saltation_ekf.md](filtering/hybrid_saltation_ekf.md): the one filtering-lineage doc about *discontinuous* motion rather than continuous motion - what a hybrid dynamical system/guard/reset map is, the saltation matrix that correctly propagates covariance through a discrete event compared against a fixed later reference time (including a plausible-looking formula that's actually correct for a *different* comparison, caught by finite-difference verification against the specific comparison this repo's own fixed-timestep filter needs), and an empirical finding in `saltation_matrix_ekf.py` that runs slightly against the naive expectation: the mathematically-exact saltation correction is modestly more sensitive to real tracking error than the naive reset-Jacobian-only update, not less - a small but consistent gap, quantified further in §8 once contact-detection timing itself is made uncertain.
