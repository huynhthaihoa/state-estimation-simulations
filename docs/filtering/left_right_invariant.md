...
| A sensor measures the robot's own position directly in the world frame (GPS: $h(X) = X\,b$ for a fixed body-frame point $b$, e.g. the antenna location), or "group-affine" IMU [dead-reckoning](../optimization/factor_graph.md#2-why-do-we-need-it) models (pose+velocity, as in legged-robot contact-aided IEKF) | often **right**-invariant - substituting $X=\eta_R\hat X$ gives $h(X) = \eta_R\cdot h(\hat X)$, so the innovation's Jacobian depends only on the *predicted* measurement, not on the rotation estimate |
