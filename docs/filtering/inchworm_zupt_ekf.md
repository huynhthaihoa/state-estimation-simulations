...
<!-- `unified_phd_plan.md` (item 3) makes an observation specific to inchworm-like locomotion: "during the anchored/dwell portion of an inchworm cycle the robot is (by definition) stationary — close to free ground truth for a zero-velocity/zero-angular-rate pseudo-measurement (ZUPT/ZARU)... during the anchor-release/extend burst neither holds."  -->
...
**Scope, stated up front**: this is deliberately a small slice of the real idea. It models translation only — zero-velocity updates (ZUPT), not the full zero-angular-rate/orientation story (ZARU) [`hybrid_saltation_ekf.md`](hybrid_saltation_ekf.md) mentions as a Stretch item — since that needs an orientation state this toy doesn't carry. It also assumes the gait schedule (when anchor/extend happen) is *known*, not detected. That second question — what happens once the schedule itself is uncertain — is exactly the subject of [`hybrid_saltation_ekf.md` §8](hybrid_saltation_ekf.md#8-quantifying-open-consideration-1-contact-detection-timing-jitter), which this doc leans on rather than repeating.
...
<!-- ## 4. Takeaway for the real Year 2 contact-phase factor

This toy's `never`/`always`/`phase_conditional` split is a minimal stand-in for exactly the design question `unified_phd_plan.md`'s open considerations raise about the single planned contact-phase GTSAM factor: which measurement terms are active should be gated by phase, built into that one factor rather than as a second novel factor. The finding above is a concrete argument for taking that gating seriously rather than treating it as a minor detail — getting it wrong doesn't just cost accuracy during the wrong phase, it can poison consistency for a long time afterward, and getting it right doesn't come free of calibration cost either. -->
