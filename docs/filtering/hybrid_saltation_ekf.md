...
is exactly $\Xi_{\text{own-time}}$ plus the extra correction term that formula was missing. This is what `saltation_matrix` computes, and $\Phi_{\text{after}}\,\Xi\,\Phi_{\text{before}}$ matches the finite-difference ground truth of the whole fixed-dt map to $\sim\!7\times10^{-6}$ (limited by the finite-difference step itself, not this formula) - see this module's tests for the check.
...
With that fixed, sweeping $\sigma_{\text{detect}}$ (with $b_{\text{detect}} = 0$, i.e. jitter only, no systematic delay) at $dt = 0.02\,\text{s}$, averaged over seeds 0/1/3 (mean NEES over the same 5-tick post-bounce window as §6, individual seeds agreeing to within ~5% of each other at every setting):

| $\sigma_{\text{detect}}$ (fraction of $dt$) | EKF (naive) | EKF (saltation) | saltation / naive |
| --- | --- | --- | --- |
...
Over one cycle, $DP$ is a product of continuous-flow Jacobians and saltation matrices, one of each per phase and event the cycle passes through: $DP \approx \Phi_3\,\Xi_2\,\Phi_2\,\Xi_1\,\Phi_1$ for a cycle with three flow phases and two events in between, where each $\Phi_i$ is the flow's own Jacobian over phase $i$ and each $\Xi_i$ the saltation matrix at event $i$. Unlike `step_hybrid`'s fixed-dt comparison (§3-§4), a Poincaré return map samples the state at the *same kind of event* every cycle - so every reference point in this composition, including each intermediate $\Phi_i$'s endpoint, is itself an event-crossing that a perturbed trajectory reaches at its own natural time, not a shared external clock. That is the setting $\Xi_{\text{own-time}}$ from §4 (the formula this script's own `saltation_matrix` no longer computes) is built for, and it's the quantity classically used in this composition in the hybrid-systems literature this doc draws on.

This script doesn't build or verify that composition itself - it propagates one covariance forward through one fixed-dt tick at a time, which is exactly the different setting §3-§4 work out in detail - so this section stays a pointer to where saltation matrices show up next, not a second worked derivation. Anyone extending this toy into a periodic-orbit-stability tool should re-derive and numerically verify $DP$'s formula directly (the same finite-difference discipline §4 uses here), rather than assuming either this script's $\Xi$ or $\Xi_{\text{own-time}}$ carries over to a multi-phase composition without a fresh check.
...
Read this as: *starting 5cm higher behaves, to first order, like starting at the same height but already falling ≈0.0485m/s faster* - which is a language $DR$ already knows how to handle. Applying $DR$'s bounce law to that equivalent velocity gives the $\Xi_{\text{own-time}}$ prediction:
