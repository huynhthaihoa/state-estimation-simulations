...
Once you've been through all eight phases, [`docs/README.md`](docs/README.md) is the fuller index to come back to for non-linear reference use.
...
**Close with (read only, no script):** [`docs/optimization/isam2_optimization.md`](docs/optimization/isam2_optimization.md) (what selective relinearization + reordering buy you beyond what you just built), [`docs/optimization/sparse_cholesky_factorization.md`](docs/optimization/sparse_cholesky_factorization.md) (why every GN/LM solve above exploited sparsity), and [`docs/optimization/marginalization.md`](docs/optimization/marginalization.md) (sliding-window smoothing - ties MSCKF/VINS-Mono back to the elimination machinery you just learned).

---

**Why this order:** each phase's docs are exactly the prerequisite the next script needs and nothing more, complexity ramps monotonically (flat-vs-manifold → single-pose filtering → multi-pose graphs → joint pose+landmark → incremental/scalable), and the three conceptual-only docs (`vi_initialization.md`, `isam2_optimization.md`, `marginalization.md`) land right where their ideas are most load-bearing even without a script of their own.
