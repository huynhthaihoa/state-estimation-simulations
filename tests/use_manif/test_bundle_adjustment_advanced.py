...
@pytest.fixture
def bundle_adjustment_advanced(import_module, use_manif_dir):
    return import_module(use_manif_dir, "bundle_adjustment_advanced")
...
def test_generate_ground_truth_trajectory_shapes_and_validity(bundle_adjustment_advanced):
    ...
    for T in T_true:
        ...
        assert np.isclose(np.linalg.norm(T.translation()[0:2]), 15.0, atol=1e-8)


def test_generate_landmark_corridor_shape(bundle_adjustment_advanced):
    baa = bundle_adjustment_advanced
...
def test_build_observations_raises_on_min_observations_below_2(bundle_adjustment_advanced):
    baa = bundle_adjustment_advanced
...
def test_triangulate_and_refine_landmark_recovers_true_point(bundle_adjustment_advanced):
    baa = bundle_adjustment_advanced
    T_true = baa.generate_ground_truth_trajectory(6, 15.0, 90.0)
    T_obs_list = T_true[2:5]
    P_true_point = T_obs_list[1].act(np.array([0.2, 0.1, 4.0]))
...
def test_point_depth_and_cheirality(bundle_adjustment_advanced):
    baa = bundle_adjustment_advanced
    T_cam = manif.SE3.Identity()
    ...
    identity_quat = np.array([0.0, 0.0, 0.0, 1.0])  # xyzw identity rotation
    T_cam2 = manif.SE3(np.array([0.0, 0.0, 20.0]), identity_quat)  # facing the same way, further along +z
...
def test_cull_invalid_points_removes_only_invalid(bundle_adjustment_advanced):
    baa = bundle_adjustment_advanced
    T_cam = manif.SE3.Identity()
    T_est = {0: T_cam}
...
def test_pose_errors_dict_zero_for_identical_poses(bundle_adjustment_advanced):
    baa = bundle_adjustment_advanced
    T_true = baa.generate_ground_truth_trajectory(6, 15.0, 90.0)
    T_est = {i: T for i, T in enumerate(T_true)}
...
def test_pose_errors_dict_subset_of_keyframes(bundle_adjustment_advanced):
    baa = bundle_adjustment_advanced
    T_true = baa.generate_ground_truth_trajectory(6, 15.0, 90.0)
    T_est = {i: T for i, T in enumerate(T_true)}
    # Build a genuinely new pose (rather than mutating T_true[3] in place,
    # which manif.SE3's setters would do to the *shared* underlying object)
    # with the same rotation but a translation shifted by 1m along x.
    shifted_translation = T_true[3].translation() + np.array([1.0, 0.0, 0.0])
    quat = T_true[3].coeffs()[3:7]
    T_est[3] = manif.SE3(shifted_translation, quat)
...
def test_build_active_window_includes_k_and_respects_anchors(bundle_adjustment_advanced):
    baa = bundle_adjustment_advanced
...
def test_run_global_ba_reduces_reprojection_rms(bundle_adjustment_advanced):
    baa = bundle_adjustment_advanced
    n_keyframes = 12
    ...
    T_est = {i: T + manif.SE3Tangent(rng.normal(0.0, 0.03, 6)) for i, T in enumerate(T_true)}
...
def _run_pair(baa, seed, n_keyframes=12):
    ...
    # empirically in the use_numpy counterpart of this test) so this small a
    # scene still yields plenty of covisible, triangulable landmarks -- with
    # the module's own CLI defaults (max_view_range=6, min_observations=3)
    # most seeds at this keyframe count produce *zero* triangulable
    # landmarks, which would make the Global-BA-vs-Local-only comparison
    # below vacuous (both runs trivially tie).
