...
@pytest.fixture
def imu_preintegration(import_module, use_manif_dir):
    return import_module(use_manif_dir, "imu_preintegration")
...
def test_straight_line_motion_matches_closed_form(imu_preintegration):
    ...
    # R_prev == Identity throughout), the discrete recursion telescopes exactly
    # to the continuous constant-acceleration formulas: delta_v(T) = v*T,
    # delta_p(T) = 0.5*v*T^2.
    ...
    assert np.allclose(bundle.delta_R.rotation(), np.eye(3), atol=1e-10)


def test_constant_axis_rotation_matches_manif_exp_closed_form(imu_preintegration):
    # Rotating about a fixed axis each micro-step commutes/composes additively in
    # angle, so after n steps of dt, delta_R == Exp(w * n*dt) exactly (up to
    ...
    T = n_steps * dt
    expected_R = manif.SO3.Identity().rplus(manif.SO3Tangent(w * T))
    assert np.allclose(bundle.delta_R.rotation(), expected_R.rotation(), atol=1e-8)
    # sanity: still a valid rotation
    R = bundle.delta_R.rotation()
...
def test_get_corrected_measurement_with_same_bias_is_identity(imu_preintegration):
    ...
    assert np.allclose(c_R.coeffs(), bundle.delta_R.coeffs(), atol=1e-12)
...
def test_no_motion_leaves_bundle_at_identity(imu_preintegration):
    ...
    assert np.allclose(bundle.delta_R.rotation(), np.eye(3), atol=1e-12)
