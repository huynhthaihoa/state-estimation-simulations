...
def test_se3_tangent_to_ambient_jacobian_matches_finite_difference(pointcloud_pose_tracking):
    ...
    R = manif.SE3.Identity().rplus(manif.SE3Tangent(
        np.concatenate([np.zeros(3), rng.normal(size=3) * 0.4]))).rotation()
    ...
    for i in range(6):
        ...
        # dt = R@v -- evaluated here via manif's exact rotation block for the angular part
        ...
        if i < 3:
            ...
            w = np.zeros(3)
            w[i - 3] = eps
            R_pert = R @ manif.SE3.Identity().rplus(
                manif.SE3Tangent(np.concatenate([np.zeros(3), w]))).rotation()
...
def test_vanilla_kf_transition_matrix_matches_small_angle_motion_model(pointcloud_pose_tracking):
    ...
    R_pred_expected = R @ (np.eye(3) + manif.SO3Tangent(w).hat())
