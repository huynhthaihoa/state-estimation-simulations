...
def test_position_observation_jacobian_matches_finite_difference(robot_imu_simulation):
    ...
    se3_exp = robot_imu_simulation.se3_exp
    T_est = se3_exp(rng.normal(size=6) * 0.3)
    ...
    for i in range(6):
        d = np.zeros(6)
        d[i] = eps
        T_pert = T_est @ se3_exp(d)
        J_numeric[:, i] = (T_pert[0:3, 3] - T_est[0:3, 3]) / eps
...
def test_position_observation_jacobian_angular_block_is_exactly_zero(robot_imu_simulation):
    ...
    for _ in range(5):
        T_est = robot_imu_simulation.se3_exp(rng.normal(size=6))
