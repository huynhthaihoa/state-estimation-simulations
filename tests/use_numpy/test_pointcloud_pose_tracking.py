...
@pytest.fixture
def pointcloud_pose_tracking(import_module, use_numpy_dir):
    return import_module(use_numpy_dir, "pointcloud_pose_tracking")


@pytest.fixture(scope="module")
def scenario(use_numpy_dir):
    ...
    T_init = T_true[0] @ m.se3_exp(init_offset)
...
def test_generate_ground_truth_and_data_shapes_and_initial_pose(pointcloud_pose_tracking):
    ...
    assert np.allclose(T_true[0], np.eye(4), atol=1e-12)
...
def test_motion_model_jacobians_match_finite_difference(pointcloud_pose_tracking):
    ...
    T_prev = m.se3_exp(xi0)
    # A small-magnitude twist*dt keeps the perturbation near the SE(3) identity,
    # where se3_right_jacobian is at its most numerically accurate; this keeps
    # the check tight without depending on lie_utils' behavior far from zero.
    ...
    for i in range(6):
        d = np.zeros(6)
        d[i] = eps
        T_prev_pert = T_prev @ m.se3_exp(d)
        T_pred_pert = m.motion_model(T_prev_pert, twist, dt)
        numeric_col = m.se3_log(m.se3_inv(T_pred) @ T_pred_pert) / eps
...
def test_observation_model_without_jacobian_returns_none(pointcloud_pose_tracking):
    ...
    pred, J = m.observation_model(np.eye(4), body_points, with_jacobian=False)
    assert J is None
...
def test_ekf_and_iekf_agree_to_tight_tolerance(scenario):
    # The module docstring explicitly claims run_iekf produces the *exact same*
    ...
    for T_e, T_i in zip(scenario["T_ekf"], scenario["T_iekf"]):
        assert np.allclose(T_e, T_i, atol=1e-9)
...
def test_pose_errors_zero_for_identical_trajectories(pointcloud_pose_tracking):
    ...
    T_list = [m.se3_exp(rng.normal(size=6) * 0.3) for _ in range(5)]
