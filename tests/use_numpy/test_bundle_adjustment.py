...
@pytest.fixture
def ba(import_module, use_numpy_dir):
    return import_module(use_numpy_dir, "bundle_adjustment")
...
def test_generate_ground_truth_scene_shapes_and_validity(ba):
    ...
    for T in T_true:
        ...
        assert np.isclose(np.linalg.norm(T[0:3, 3] - P_true.mean(axis=0)), 5.0, atol=1e-8)
...
def test_camera_project_jacobians_match_finite_difference(ba, seed):
    rng = np.random.default_rng(seed)
    # Build a random but valid camera pose (not the identity, so the test
    # actually exercises rotation coupling): se3_exp of a random twist is
    # guaranteed to be a valid (4,4) SE(3) matrix.
    twist = rng.normal(0.0, 0.3, 6)
    T = ba.se3_exp(twist)

    # A world point placed in front of the camera at moderate depth.
    P = T[0:3, 3] + T[0:3, 0:3] @ np.array([0.3, -0.2, 4.0])
    ...
    # J_pose: perturb the pose via a right-perturbation T @ se3_exp(eps * e_k),
    # matching the module's own convention (see camera_project's docstring).
    for k in range(6):
        d = np.zeros(6)
        d[k] = epsilon
        T_plus = T @ ba.se3_exp(d)
        T_minus = T @ ba.se3_exp(-d)
...
def test_camera_project_without_jacobians_returns_none(ba):
    T = np.eye(4)
...
def test_camera_project_matches_pinhole_formula(ba):
    T = np.eye(4)
...
def test_build_observations_raises_on_min_observations_below_2(ba):
    rng = np.random.default_rng(0)
...
def test_perturb_initial_guess_produces_valid_but_different_poses(ba):
...
def test_run_bundle_adjustment_converges_below_noise_floor(ba, small_scene):
    s = small_scene
...
def test_run_ba_landmarks_only_and_poses_only_reduce_error(ba, small_scene):
    s = small_scene
...
def test_alignment_preserves_reprojection_rms(ba, small_scene):
    s = small_scene
...
def test_compute_all_metrics_shape_and_ordering(ba, small_scene):
    s = small_scene
    rows = [
