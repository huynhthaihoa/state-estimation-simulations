...
built-in analytical Jacobians (no hand-rolled skew/exp/log math) -- except
the vanilla KF, which deliberately avoids SE(3) altogether:
...
def se3_tangent_to_ambient_jacobian(R):
    ...
    because the twist is defined in the body frame. skew(e) is built via manif's
    `SO3Tangent(e).hat()` rather than a hand-rolled formula, matching this file's convention
    (already used the same way by `run_iekf`).
    Arguments:
        R: rotation (3,3)
    Returns:
    ...
    for j in range(3):
        ...
        e[j] = 1.0
        J[9:12, j] = R @ e
        J[0:9, 3 + j] = (R @ manif.SO3Tangent(e).hat()).flatten()
    return J
...
def vanilla_kf_transition_matrix(twist, dt):
    ...
    input, so -- unlike a manif rplus Jacobian, which linearizes the exact group composition --
    applying it to each of the 12 standard basis vectors recovers the exact matrix, valid for any
    (R,t), not just the current estimate: A_k depends only on the known input, never on the
    running state.

    This first-order truncation of Exp is the real accuracy cost of using a vanilla (linear) KF
    here at all: the EKF's predict step composes T_pred = T_prev.rplus(Exp(w)) exactly (only its
    ...
    M = np.eye(3) + manif.SO3Tangent(w).hat()
    A = np.zeros((12, 12))
    for i in range(12):
        x = np.zeros(12)
        x[i] = 1.0
...
def run_vanilla_kf(T_init, P_init, u_meas, z, body_points, dt, Q_tangent, point_noise_std):
    ...
    any Jacobian re-linearization at every step. Unlike every other method in this file, its core
    recursion never touches manif's `rplus`/`rminus`/`act`/`compose` at all -- it operates
    directly on the numpy rotation matrix/translation extracted via `T.rotation()`/
    `T.translation()`, which is precisely the point: this filter deliberately steps outside the
    SE(3) group rather than working within it. manif only reappears twice: `SO3Tangent(.).hat()`
    for skew (matching `run_iekf`'s convention) and reconstructing the final `manif.SE3` pose from
    the corrected rotation matrix -- manif's SE3/SO3 expose no rotation-matrix constructor (only
    quaternion/axis-based ones), so `scipy.spatial.transform.Rotation.from_matrix(...).as_quat()`
    is used instead (verified to match manif's own [x,y,z,w] quaternion convention exactly), not
    a hand-rolled conversion.
    ...
    R0, t0 = T_init.rotation(), T_init.translation()
    ...
    for k in range(len(u_meas)):
        ...
        T_est = manif.SE3(x_est[9:12], Rotation.from_matrix(R_fixed).as_quat())
        T_list.append(T_est)
