...
Concretely, this script tracks *two* separate noise-free ground-truth
trajectories, one per estimator, each accumulated with that estimator's own
composition rule (R_gt_naive via `euler_to_R`, R_gt_exp via `so3_exp`) applied
to the same noise-free rates -- each estimator is scored against the exact,
noise-free version of its own update rule, not a single shared trajectory.

This differs from the use_manif twin, which tracks one shared, coupled SE(3)
ground truth (both estimators scored against the same T_gt) -- a real
inconsistency between the two backends, not just a cosmetic one, though it
turns out to be inconsequential to either script's own headline finding.
Concretely (default CLI args, seed 0, 20s @ dt=0.005s): R_gt_exp and
R_gt_naive drift apart from *each other* by 0.45 deg / 0.015m over the run
(purely from ZYX-Euler-angle vs. axis-angle composition applied to the same
noise-free rate each step) - so exp-map's own reported error is unaffected
either way (it's always scored against R_gt_exp/T_gt, which are the *same*
quantity by construction, and use_numpy/use_manif's final rot_err_exp
matches to ~5e-12 deg accordingly), but naive's reported error technically
depends on which ground truth backend you're comparing to: use_numpy's
final rot_err_naive (scored against R_gt_naive) is 70.78 deg vs. use_manif's
70.42 deg (scored against the coupled SE(3) truth) - a 0.36 deg gap from
this inconsistency alone. That gap is tiny next to the ~70 deg naive error
it's riding on, which is why both backends' headline numbers still land
close together and the "naive breaks down, exp-map doesn't" conclusion is
robust to it either way - but it is a real, checkable side effect of the two
backends not actually comparing against the same ground truth, worth fixing
properly (unify on one convention) if this script's numbers were ever used
for something more precision-sensitive than the qualitative comparison here.
