'''
Local + Global Bundle Adjustment, contrasted: models the *real-time system
behavior* that docs/optimization/bundle_adjustment.md Section 13 describes
but bundle_adjustment.py's batch solvers don't -- a camera moving
...
covisible neighbors + fixed anchor keyframes), instead of jointly
re-solving the whole map every time. A periodic Global BA pass (the whole
map, every --global-ba-interval keyframes) runs alongside it as the
"occasional Global BA pass" Section 13 says corrects Local BA's drift --
hence "advanced": unlike bundle_adjustment.py, this one script contains
both, run back to back for direct comparison.
...
(the doc's own Section 13 diagram -- this script implements exactly this.)

Scene: keyframes move along a forward-facing arc ("corridor") instead of
bundle_adjustment.py's static look-at-centroid camera arc, and landmarks are
scattered near the path centerline -- so a landmark is only geometrically
visible from a short run of nearby keyframes and covisibility clusters
emerge from geometry + field-of-view, not an artificial visibility-lifetime
rule.

Each keyframe arrives with a front-end pose estimate built by chaining a
noisy version of the true frame-to-frame motion onto the previous estimate
(simulate_frontend_trajectory) -- the same dead-reckoning-style compounding
used by pose_graph.py's generate_ground_truth_trajectory/simulate_noisy_edges
pair, re-derived here because it's what actually causes drift to accumulate:
independent per-frame noise would not.

Two solvers make Section 13's "windowing vs. batch" and "bounded vs. growing
cost" claims concrete:

  - run_local_ba_step: solves one bounded active window (a handful of active
    keyframe poses + active landmarks, with covisible-but-inactive keyframes
    held fixed as rigid anchors) -- this runs on *every* keyframe, and its
    cost should stay roughly flat regardless of total map size.

  - run_global_ba: solves the whole map built so far in one joint dense
    system -- structurally the same as bundle_adjustment.py's
    run_bundle_adjustment, run here every --global-ba-interval keyframes as
    the "occasional Global BA pass" Section 13 says corrects Local BA's
    accumulated drift. Its cost grows with total map size.
...
weight on every reprojection residual cancels out of the normal equations,
so this script's residuals are the doc's own plain sum ||z-pred||^2).

run_incremental_local_ba runs the whole keyframe-by-keyframe loop once with
use_global_ba=False (pure Local BA, drift never corrected) and once with
use_global_ba=True (Local BA + periodic Global BA), off the *same*
ground-truth scene/observations. Which (keyframe, landmark) pairs are ever
*visible* -- and so covisibility, active/fixed window membership, and when
a landmark first reaches min_observations -- depends only on ground-truth
geometry, so both runs see the same windows early on. Once Global BA first
corrects a pose, the two runs' numeric T_est/P_est diverge, which can
occasionally shift a borderline triangulation's cheirality outcome
...
Hand-rolled SE(3) math (skew/exp/inverse) comes from lie_utils.py, shared
with bundle_adjustment.py, pointcloud_pose_tracking.py and pose_graph.py.
'''
...
def generate_ground_truth_trajectory(n_keyframes, path_radius, arc_span_deg):
    ...
    Returns:
        T_true: list of n_keyframes ground-truth camera poses (4,4), in time order
    """
    ...
    for a in angles:
        ...
        T = np.eye(4)
        T[0:3, 0:3] = forward_facing_rotation(forward)
        T[0:3, 3] = cam_pos
        T_true.append(T)
    return T_true
...
def camera_project(T, P, K, with_jacobians=False):
    ...
    u = fx*x/z + cx, v = fy*y/z + cy. Identical to bundle_adjustment.py's
    camera_project (duplicated rather than imported -- this codebase never
    imports across sibling scripts, only through the shared lie_utils.py).
    Arguments:
...
def simulate_frontend_trajectory(T_true, relative_pose_noise_std, rng):
    ...
    pattern as pose_graph.py's simulate_noisy_edges/run_dead_reckoning; an
    independent per-frame perturbation (as in bundle_adjustment.py's
    perturb_initial_guess) would not compound and so would not drift.
    Arguments:
        T_true: list of ground-truth camera poses (4,4), in time order
        relative_pose_noise_std: std-dev of the se3 twist noise added to each relative motion (mixed m/rad)
        rng: numpy random number generator
    Returns:
        T_init: list of front-end pose estimates (4,4), in time order
    """
    T_init = [T_true[0].copy()]
    for k in range(1, len(T_true)):
        T_rel_true = se3_inv(T_true[k - 1]) @ T_true[k]
...
def point_depth(T, P):
    ...
    return (T[0:3, 0:3].T @ (P - T[0:3, 3]))[2]
...
def cull_invalid_points(T_est, P_est, observers_by_landmark):
    ...
    Arguments:
        T_est: dict {keyframe_idx: (4,4) pose}, current estimates
...
def run_local_ba_step(T_est, P_est, active_keyframes, fixed_keyframes, active_points,
    ...
    Arguments:
        T_est: dict {keyframe_idx: (4,4) pose}, current estimates (mutated for active keyframes)
...
def run_windowed_gn_lm(T_est, P_est, pose_idx, point_idx, obs_list, K, gn_tol, gn_max_iters):
    ...
    *increases* the error and then diverge explosively on the next
    ...
    Arguments:
        T_est: dict {keyframe_idx: (4,4) pose}, current estimates (mutated for keys in pose_idx)
    ...
    for _ in range(gn_max_iters):
        ...
        for _retry in range(10):
            ...
            T_trial = {i: T_est[i] @ se3_exp(delta[6 * k:6 * k + 6]) for i, k in pose_idx.items()}
...
def run_global_ba(T_est, P_est, keyframe_ids, point_ids, observers_by_landmark, K,
    ...
    Arguments:
        T_est: dict {keyframe_idx: (4,4) pose}, current estimates (mutated for non-anchor keyframes)
...
def pose_errors_dict(T_true, T_est, keyframe_ids):
    ...
    Arguments:
        ...
        T_est: dict {keyframe_idx: (4,4)} of estimated camera poses
    ...
    for idx, i in enumerate(ids):
        rot_err[idx] = np.degrees(rotation_geodesic_error(T_true[i][0:3, 0:3], T_est[i][0:3, 0:3]))
        pos_err[idx] = np.linalg.norm(T_true[i][0:3, 3] - T_est[i][0:3, 3])
...
def main():
    ...
    true_pos = np.array([T[0:3, 3] for T in T_true])
    hybrid_pos = np.array([T_hybrid[i][0:3, 3] for i in range(args.n_keyframes)])
