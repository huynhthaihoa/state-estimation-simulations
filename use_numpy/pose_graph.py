...
def run_pose_graph_optimization(T_init_list, constraints, info_matrix, damping, gn_tol, gn_max_iters):
    ...
    pattern bundle_adjustment_advanced.py's run_windowed_gn_lm uses, rather
    than a fixed damping term added once per iteration regardless of
    whether the resulting step helps. A fixed-damping scheme only happens
    to work when the problem is already well-conditioned enough that every
    step helps (true for this script's own well-anchored toy scenes); it
    isn't a substitute for an accept/reject criterion in general, and a
    fixed-damping *baseline* solver used purely as an "eventually converges
    however long it takes" reference (e.g. pose_graph_incremental.py's
    run_batch_streaming) needs the adaptivity to actually reach that
    reference point in a bounded number of iterations.
    Arguments:
    ...
    def cost(T_list):
        ...
        for idx_i, idx_j, Z_ij in constraints:
            e_vec = se3_log(se3_inv(T_list[idx_i] @ Z_ij) @ T_list[idx_j])
    ...
    for it in range(gn_max_iters):
        ...
        for _retry in range(10):
            ...
            T_trial = [T_est[k] @ se3_exp(delta[6 * k:6 * k + 6]) for k in range(n_poses)]
