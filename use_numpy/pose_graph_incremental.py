'''
Incremental (square-root SAM) pose-graph solving, contrasted with the batch
Levenberg-Marquardt/Gauss-Newton relaxation in pose_graph.py -- the mechanism
docs/optimization/isam_optimization.md Section 3 describes (Dellaert & Kaess
2006's square-root-information factorization, Kaess et al. 2008's iSAM) but
pose_graph.py's batch solver doesn't: absorbing one new measurement into an
existing factorization via Givens-rotation row insertion (`qr_insert_row`,
utils.py) instead of rebuilding the whole linear system from scratch.
...
from lie_utils import se3_exp, se3_log, se3_inv, se3_adjoint, compute_se3_inv_right_jacobian
...
def linearize_edge(Xi, Xj, Z_ij):
    """Residual and Jacobians of e_ij = Log((Xi @ Z_ij)^-1 @ Xj) wrt Xi and
    Xj -- the same Jacobian-chaining pattern as
    pose_graph.py's run_pose_graph_optimization, factored out here since both
    the incremental-insertion path and the full-relinearization path need it.
    Arguments:
        Xi, Xj: current pose estimates at the edge's two endpoints (4,4)
        Z_ij: measured relative pose (4,4)
    Returns:
        J_i: (6,6) d e_ij / d Xi
        J_j: (6,6) d e_ij / d Xj
        e_vec: (6,) residual
...
def full_relinearize(x_lin, odom_seen, loop_seen, info_matrix, anchor_weight):
    ...
    Arguments:
        x_lin: list of pose estimates, the linearization point (4,4 each)
...
def relinearize_to_convergence(x_current, odom_seen, loop_seen, info_matrix, anchor_weight,
    ...
    Arguments:
        x_current: list of pose estimates to relinearize around (4,4 each)
    ...
    Returns:
        x_current: converged pose estimates (4,4 each)
    ...
    for _ in range(gn_max_iters):
        ...
        x_current = [x_current[k] @ se3_exp(delta[6 * k:6 * k + 6]) for k in range(len(x_current))]
...
def run_incremental_pose_graph(gt_poses, odometry_constraints, loop_constraints, info_matrix,
    ...
    Returns:
        x_est: list of estimated poses (4,4) after all nodes/edges have streamed in
    ...
    def read_out():
        delta = solve_triangular(R, d)
        return [x_lin[k] @ se3_exp(delta[6 * k:6 * k + 6]) for k in range(len(x_lin))]
...
def run_batch_streaming(gt_poses, odometry_constraints, loop_constraints, info_matrix,
    ...
    Returns:
        T_final: list of estimated poses (4,4) after all nodes/edges have streamed in
